...
$error              = 0;
$query              = '';
$headerTabla        = "<thead><th>Articulo</th><th>Producto</th><th>Precio</th><th>categoria</th></thead>";
$columnaImagen      = 0;
...
<div class="col-md-12">
    <form action="index.php?op=productos&accion=importar" method="post" enctype="multipart/form-data">
        <h3>Importar productos de Excel a la Web (<a href="upload/modelo.xlsx" target="_blank">descargar modelo</a>)</h3>
        <hr/>
        <div class="row">
            <div class="col-md-6">
    ...
    if (isset($_FILES['uploadFile']['name']) && $_FILES['uploadFile']['name'] != "") {
        ...
        if (in_array($ext, $allowedExtensions)) {
            ...
            if ($isUploaded) {
                ...
                echo $headerTabla;
                $query = "insert into `productos` (`articulo`,`imagen`,`titulo`,`descripcion`,`precio1`,`categoria`) VALUES ";
                for ($row = 2; $row <= $total_rows; $row++) {
                    ...
                    if ($single_row[0][0] != '') {
                        ...
                        foreach ($single_row[0] as $key => $value) {
                            ...
                            $query .= "'" . trim($value) . "',";
                            if ($key == $columnaImagen) {
                                $query .= "'archivos/productos/recortadas/" . trim($value) . ".jpg',";
                            }
                            if ($key == $columnaDescripcion) {
                                $query .= "'" . trim($value) . "',";
                            }
                        }
...
if (isset($_POST["subir"])) {
    if (!empty($_SESSION["query"])) {
        mysqli_query($con, "DELETE FROM `productos` WHERE `tipo` = 0");
